#!/usr/bin/env node
/**
 * CI gate: pricing.tsx and sync-support-kb.ts must agree on Developer observe-only.
 */
...
const checks = [
  {
    name: 'Developer governed actions none (pricing page)',
    ok: /Governed actions[\s\S]*?value:\s*"None"/i.test(pricingTsx),
  },
  {
    name: 'Developer observe-only (pricing page)',
    ok: /observe only/i.test(pricingTsx),
  },
  {
    name: 'Developer no governed actions (KB sync)',
    ok: /Governed actions:\s*none\s*\(observe only/i.test(syncKb),
  },
  {
    name: 'Developer $0 free (KB sync)',
    ok: /\$0.*free forever/i.test(syncKb),
  },
  {
    name: 'No stale 50/mo on Developer in KB sync',
    ok: !/(?:Developer|Observer)[\s\S]{0,400}50\/mo/i.test(syncKb),
  },
]
